...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'travel-blog' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( esc_html__( 'Older posts', 'travel-blog' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts', 'travel-blog' ) ); ?></div>
			<?php endif; ?>
...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'travel-blog' ); ?></h2>
		<div class="nav-links">
			<?php
...
function travel_blog_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'travel-blog' ), esc_html__( '1 Comment', 'travel-blog' ), esc_html__( '% Comments', 'travel-blog' ) );
		echo '</span>';
	}

	edit_post_link( esc_html__( 'Edit', 'travel-blog' ), '<span class="edit-link">', '</span>' );
}
endif;
...
function the_archive_title( $before = '', $after = '' ) {
	if ( is_category() ) {
		$title = sprintf( esc_html__( 'Category: %s', 'travel-blog' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		$title = sprintf( esc_html__( 'Tag: %s', 'travel-blog' ), single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		$title = sprintf( esc_html__( 'Author: %s', 'travel-blog' ), '<span class="vcard">' . get_the_author() . '</span>' );
	} elseif ( is_year() ) {
		...
		$title = sprintf( esc_html__( 'Archives: %s', 'travel-blog' ), post_type_archive_title( '', false ) );
	} elseif ( is_tax() ) {
